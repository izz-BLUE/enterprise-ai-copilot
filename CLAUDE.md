...
- P3-5A / P3-5B2a：Python 图只为已由 Java 成功提交且带本地 ExpenseClaim `request_id` 的报销确认追加持久化 `external_wait_node(interrupt)`。B2a 的 Mock OA SQLite 先提交 PENDING → APPROVED/REJECTED，再发送不含 status 的 HMAC-SHA256 webhook；Java 验证原始 body、5 分钟 timestamp 后 GET OA 权威状态，以幂等且禁止回退的方式更新 ExpenseClaim。普通 Chat 不跨过 interrupt；B2a 不调用 external resume，Graph 仍为 `WAITING_EXTERNAL`。后续 external resume 仍严格绑定 actor 与 wait/execution/action/local-request，并只用 `Command(resume)` 消费 active external decision；它不改变 Java business action 或 Memory terminal。
