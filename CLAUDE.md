...
- P3-5A 只在 Python 图中为已由 Java 成功提交且带本地 ExpenseClaim `request_id` 的报销确认追加第二个持久化 `external_wait_node(interrupt)`。普通 Chat 不跨过该 interrupt；external resume 严格绑定 actor 与 wait/execution/action/local-request，允许跨 business date，并只用 `Command(resume)` 消费 active external decision。external result 已落盘后的 finalizer crash 才允许相同 payload `invoke(None)`，completed 相同 replay 是 no-op。它不改变 Java business action 或 Memory terminal，也不包含 Java/Mock OA/webhook 实现。
