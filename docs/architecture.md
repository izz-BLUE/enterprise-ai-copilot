...
- **P3-1 / P3-2 / P3-3 执行快照**：`checkpoint_runtime` 在 FastAPI 启动时按 `LANGGRAPH_CHECKPOINT_MODE` 创建 `ConnectionPool`、显式严格 `JsonPlusSerializer`、调用官方 `PostgresSaver.setup()`，并一次性编译两套带 Checkpointer 的图；POSTGRES 模式每个节点以 `durability="sync"` 写入。`rt_<hash>:planner-v1` 与 `rt_<hash>:deterministic-v1` 相互隔离。Planner-first 新执行从 START 写入 strict `execution_recovery` marker；同一 thread 的下一次 exact request 先只读取 latest `snapshot`，以 `snapshot.next != ()` 判定未完成，再校验 marker v1、日期 anchor、单一合法 pending node 与 Tool replay policy，通过后使用 `graph.invoke(None)` 从 latest head 继续。Resume 不重建 initial state、不 hydrate P3-2 history，并重新注入当前 Java Runtime Context；legacy deterministic graph 不启用自动 Resume。`execution_history_policy` 只把当前请求中 `travel_record_tool` / `invoice_verify_tool` 的成功 Observation 白名单归一化为 `CONTEXT_ONLY` 摘要，在 `finalize_node` 中合并后随最终 Checkpoint 保存。
...
P3-2 不实现历史事实的 TTL / revalidation；P3-3 的 Crash Resume 不依赖 ACTIVE Memory。Fresh request 的 `tool_history=[]`，普通跨请求连续性使用 `execution_history`；同一次未完成执行 Resume 使用 Checkpoint 中原样保留的 `tool_history`、`execution_history`、计数、Planner decision 和 execution marker。只有 exact same question、同一 `business_date` anchor、合法 replay-safe pending Tool 才可自动恢复；completed execution 永远 Fresh。request/date conflict、旧 marker、interrupt、未知/并行 pending node 和 unsafe Tool 均 fail-closed 为 HTTP 409。真实外部副作用 Tool 必须具备业务幂等，或保持 `resume_safe=False`。Checkpoint retention / pruning 同样暂缓。
