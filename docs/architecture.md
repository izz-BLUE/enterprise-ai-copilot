...
- **P3-5A Durable WAITING_EXTERNAL foundation**：不改变 P3-4 的 Java authority 与 Memory terminal 语义。报销 `CONFIRMED + EXPENSE_CLAIM + local ExpenseClaim request_id` 在同一 execution 内追加 `prepare_external_wait_node → external_wait_node(interrupt)`；`ExternalWaitMarker.wait_id` 由 execution id 与本地 request id 确定性派生，不保存 raw identity、权限、日期、trace、nonce、credential 或 webhook payload。普通 Chat 看到合法 external interrupt 时优先返回 `WAITING_EXTERNAL`，不比较原 question/date、不 hydrate history、不启动 Fresh 或 `invoke(None)`。内部 external resume 重新注入当前 Runtime Context，严格校验 actor 与四项 correlation 后用 `Command(resume=...)`；外部 decision 已 checkpoint、只剩 finalizer 时才以相同 payload 允许 `invoke(None)`，完成后相同 replay 为 no-op。external approved/rejected 只是 Expense Domain 后续生命周期结果，不回滚 Java `business_action=SUCCEEDED`，也不重新激活 Memory；P3-5A 不包含 Java、Mock OA 或 webhook 实现。
