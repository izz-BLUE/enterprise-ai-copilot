...
- **P3-5A / P3-5B1 Durable WAITING_EXTERNAL**：不改变 P3-4 的 Java authority 与 Memory terminal 语义。报销 `CONFIRMED + EXPENSE_CLAIM + local ExpenseClaim request_id` 在同一 execution 内追加 `prepare_external_wait_node → external_wait_node(interrupt)`；`ExternalWaitMarker.wait_id` 由 execution id 与本地 request id 确定性派生，不保存 raw identity、权限、日期、trace、nonce、credential 或 webhook payload。Java 只在本地 ExpenseClaim / BusinessAction / Memory 已提交，且 Python HITL resume 返回 marker 后，严格核对 schema、type、request、execution 和重算的 wait id；随后持久化 immutable `external_wait_id`。`ExpenseExternalApprovalCoordinator` 在事务外调用独立的 Mock OA，仅发送 Expense 业务字段和确定性 `Idempotency-Key`，OA 成功后才原子保存 `external_provider` / `external_request_id` 并令 ExpenseClaim `SUBMITTED → WAITING_APPROVAL`。请求失败或响应丢失不回滚任何 Java 权威状态；后台仅重试缺失 OA request id 的已绑定 SUBMITTED 记录，Mock OA 的持久化幂等键防止重复。若 Python 已持久化进入 `WAITING_EXTERNAL`，但 Java 在持久化 `external_wait_id` 前失败，当前版本依赖既有幂等用户重试 / 运维恢复；通用自主对账机制作为生产强化延期，B1 不实现。普通 Chat 不跨过 external interrupt；Webhook、HMAC、审批轮询、approve/reject 与 external `Command(resume)` 仍未实现。
