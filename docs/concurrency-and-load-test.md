...
| Java LangGraph | `AgentRuntimeThreadExecutionGuard` | 同一 runtime thread 仅一个请求，忙时立即 429 | 串行化 Memory Read → Python latest recovery inspection/Graph invoke → PendingAction/Memory persist 的完整 conversation 生命周期 |
...
LangGraph conversation guard 是 Java 进程内保护，不创建每个 thread 的永久锁对象；当前单实例部署足够。Python 侧另有 `active_thread_ids` 保护 latest recovery inspection 到最终 Checkpoint 的区间。多 Java/Python 实例或水平扩展需要分布式 conversation/thread lease，本阶段不实现。
