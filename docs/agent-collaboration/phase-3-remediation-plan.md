...
| FIX-003 | Python 服务可被直接访问（端口 8000 无控制） | Security P0-3 | A1 | fix/python-access | `application.properties` + 部署文档 | Python 仅允许 localhost 访问，或添加内部 API Key | ✅ 是 | 🟡 部署准备说明已完成（详见 `docs/deployment-readiness.md`），上线前必须实施访问控制 |
