...
| 事实 | 校验 |
|---|---|
| Trip | ID 存在、员工归属正确、状态 `APPROVED`、当前 start/end 日期有效，并重新计算住宿夜数 |
| Invoices | ID 集合精确匹配、归属正确、`valid=true`、`duplicate=false`、金额和 category 未变 |
| Amount | Java 根据当前 facts 确定性重算 claimed/reimbursable，并与 Proposal 一致 |

结果分三类：

- **Fresh**：进入 Java 本地事务，写入 ExpenseClaim/Items；
- **Stale**：Action=`FAILED`、Memory=`ABANDONED`、HITL=`REJECTED`，不创建 ExpenseClaim，正常路径通过 REJECTED resume 到 Graph `END`；若 stale 终态已在 Java 提交但 Python resume 失败，Java `FAILED` 保留，重复 Confirm 不重新校验 OA 或改变 Java 状态，只重试同一个确定性的 REJECTED continuation；没有自主 stale-HITL worker；
...
| Java 权威 | PendingAction | ExpenseClaim + Mock OA 权威 GET |
| Python marker（标记） | `BUSINESS_ACTION_CONFIRMATION` | `EXPENSE_APPROVAL` |
| Python endpoint（端点） | `/agent/langgraph/hitl/resume` | `/agent/langgraph/external/resume` |
| resume payload（恢复载荷） | Action decision/status | OA decision/status/request |
| 普通 Chat | 不得跨过 | 不得跨过 |
...
GET  /api/expense-approvals/{requestId} → 当前 status
...
- `WAITING_EXTERNAL`：等待 Java 权威 decision；
