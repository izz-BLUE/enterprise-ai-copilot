...
Memory 相关行为包含在当前 Python full suite 与 Java integration baseline 中：Python full suite `1512 passed + 34 expected skips`；PostgreSQL checkpoint/recovery/HITL/external resume 合计 `34 passed, 0 skipped`。Purchase persistence proof 另有 `4 passed`，不进入 Memory 生命周期。这些数字是项目接受基线，不把本页变成一次新的生产容量声明。
