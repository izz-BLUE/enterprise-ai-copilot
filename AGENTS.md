...
- **P3-1 执行快照边界**：Java 基于可信 `VerifiedIdentity.userId()` 与已解析 `conversationId` 生成 `X-Agent-Thread-Id`；Python POSTGRES 模式在启动时复用 `ConnectionPool` / `PostgresSaver` / 两套持久化图，节点 `durability="sync"` 落盘。Checkpoint 只记录执行现场，不是业务事实或权限来源；新请求仍以完整 `AgentState` 从 START 运行，不含 resume / interrupt / 跨请求 tool history。
