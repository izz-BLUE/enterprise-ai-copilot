...
- **Python Agent Graph**：main 同时保留两套互斥状态图。`AGENT_LOOP_ENABLED=false`（仓库部署默认）走 legacy Router-first：`safety → router → rag|eval|action|refuse`，意图路由 + 规则工具；`AGENT_LOOP_ENABLED=true` 走 Planner-first：`safety → planner ⇄ tool_executor`，Planner 拥有规划权、无最终业务执行授权，预算受 `MAX_PLANNER_STEPS=5` / `MAX_TOOL_CALLS=3` 收敛。Planner-first **最多支持 5 个 Tool**，实际可见集合由程序层按权限动态收缩，**模型不能自行扩大 Tool 权限**：
...
- **公网状态约束**：仓库对公网实际运行版本无证据，文档统一表述"仓库部署默认 legacy Router-first"；公网是否启用 Planner-first / 受控业务动作以运维 `.env` 为准，本文件不据此做能力宣称。
